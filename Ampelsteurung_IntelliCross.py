...
class _Linse(tk.Canvas)
    def _init_(self, parent, farbe str, groesse int = 52, hg str = #111122)
        super()._init_(
            parent,
            width=groesse,
            height=groesse,
...
class RPiZAGui
    def _init_(self, root tk.Tk, steuerung RPiZA, mock bool)
        self.root = root
    ...
    def _baue_konfiguration(self, parent)
        ...
        tk.Label(
            parent,
            text=fFzA-Grünphase [{FZA_GRUEN_MIN}–{FZA_GRUEN_MAX} s],
            bg=_HINTERGRUND,
            fg=#cccccc,
            width=36,
            anchor=w,
        ...
        tk.Label(
            parent,
            text=fFgA-Grünphase [{FGA_GRUEN_MIN}–{FGA_GRUEN_MAX} s],
            bg=_HINTERGRUND,
            fg=#cccccc,
            width=36,
            anchor=w,
        ...
        try
            ...
            if FZA_GRUEN_MIN = v_fza = FZA_GRUEN_MAX
                ...
                self._fehler_fza.config(
                    text=fWert {v_fza} ungültig — Bereich [{FZA_GRUEN_MIN}, {FZA_GRUEN_MAX}]
                )
                ok = False
        ...
        try
            ...
            if FGA_GRUEN_MIN = v_fga = FGA_GRUEN_MAX
                ...
                self._fehler_fga.config(
                    text=fWert {v_fga} ungültig — Bereich [{FGA_GRUEN_MIN}, {FGA_GRUEN_MAX}]
                )
                ok = False
    ...
    def _aktualisiere(self)
        ...
        if zustand == RPiZA.S_FEHLER
            self._zustand_var.set(
                fFEHLER — {self.steuerung.fehler_grund} · alle Ampeln Rot
            )
        else
