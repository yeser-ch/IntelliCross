...
class _Linse(tk.Canvas):
    def __init__(self, parent, farbe: str, groesse: int = 52, hg: str = "#111122"):
        super().__init__(
            parent, width=groesse, height=groesse, bg=hg, highlightthickness=0,
        )
        m = 5
        self._oval = self.create_oval(
            m, m, groesse - m, groesse - m, fill=_LED_AUS, outline="#444", width=2,
        )
...
class RPiZAGui:
    ...
    def _baue_gui(self):
        ...
        tk.Label(
            r, text="RPiZA v3.0 — Fußgänger-Zuflussregelungsanlage",
            font=("Helvetica", 16, "bold"), bg=_HINTERGRUND, fg="white",
        ).pack(pady=(12, 2))

        tk.Label(
            r, text="StVO §37 · RiLSA 2015 · Lights! GmbH",
            font=("Helvetica", 9), bg=_HINTERGRUND, fg="#888888",
        ).pack(pady=(0, 2))

        tk.Label(
            r, text="Sicherheitsüberwachung aktiv — Watchdog + Verträglichkeit",
            font=("Helvetica", 8), bg=_HINTERGRUND, fg="#5a8a5a",
        ).pack(pady=(0, 4))

        self._zustand_var = tk.StringVar(value=_ZUSTAND_TEXT[RPiZA.S_CONFIG])
        tk.Label(
            r, textvariable=self._zustand_var, font=("Helvetica", 11),
            bg=_HINTERGRUND, fg="#aaaaff", width=54,
        ).pack(pady=(0, 2))

        self._timer_var = tk.StringVar(value="Phase: 0 s")
        tk.Label(
            r, textvariable=self._timer_var, font=("Courier", 11),
        ...
        self._btn_start = tk.Button(
            btn_frame, text="Start", width=12, bg="#1a6b3a", fg="white",
            command=self._start,
        )
        self._btn_start.grid(row=0, column=0, padx=6)

        self._btn_stop = tk.Button(
            btn_frame, text="Stop / Aus", width=18, bg="#6b1a1a", fg="white",
            command=self._stop, state="disabled",
        )
        self._btn_stop.grid(row=0, column=1, padx=6)

        self._btn_fehler = tk.Button(
            btn_frame, text="Fehler / Fail-Safe", width=18, bg="#7a3a00", fg="white",
    ...
    def _baue_fza_panel(self, parent, col):
        ...
        tk.Label(
            f, text="FzA\nFahrzeugampel", bg=_FZA_BG, fg="white",
    ...
    def _baue_fga_panel(self, parent, col, label):
        ...
        f.grid(row=0, column=col, padx=14, pady=4)
        tk.Label(
            f, text=label, bg=_FGA_BG, fg="white",
            font=("Helvetica", 9, "bold"),
        ).pack(pady=(8, 2))
    ...
    def _baue_konfiguration(self, parent):
        tk.Label(
            parent, text="Operator-Konfiguration", font=("Helvetica", 12, "bold"),
            bg=_HINTERGRUND, fg="#ffcc44",
        ).grid(row=0, column=0, columnspan=3, pady=(0, 8))

        tk.Label(
            parent, text=f"FzA-Grünphase [{FZA_GRUEN_MIN}–{FZA_GRUEN_MAX} s]:",
            bg=_HINTERGRUND, fg="#cccccc", width=36, anchor="w",
        ...
        self._fehler_fza = tk.Label(
            parent, text="", bg=_HINTERGRUND, fg="#ff6666", width=38, anchor="w",
        )
        self._fehler_fza.grid(row=1, column=2, sticky="w")

        tk.Label(
            parent, text=f"FgA-Grünphase [{FGA_GRUEN_MIN}–{FGA_GRUEN_MAX} s]:",
            bg=_HINTERGRUND, fg="#cccccc", width=36, anchor="w",
    ...
    def _start(self):
        ...
        self._btn_stop.config(text="Stop / Aus")
        self._btn_fehler.config(state="normal")
    ...
    def _stop(self):
        ...
        self._btn_stop.config(state="disabled", text="Stop / Aus")
        self._btn_fehler.config(state="normal")
    ...
    def _fehler_ausloesen(self):
        ...
        self._eingabe_fga.config(state="disabled")
        self._btn_start.config(state="disabled")
        self._btn_stop.config(state="normal", text="Fehler quittieren / Aus")
    ...
    def _aktualisiere(self):
        ...
        if zustand == RPiZA.S_FEHLER:
            ...
            self._btn_stop.config(state="normal", text="Fehler quittieren / Aus")
            self._btn_fehler.config(state="disabled")
        elif zustand == RPiZA.S_AUS:
            self._zustand_var.set(_ZUSTAND_TEXT[RPiZA.S_AUS])
            self._btn_start.config(state="normal")
            self._btn_stop.config(state="disabled", text="Stop / Aus")
            self._btn_fehler.config(state="normal")
            self._eingabe_fza.config(state="normal")
            self._eingabe_fga.config(state="normal")
        elif zustand == RPiZA.S_CONFIG:
            self._zustand_var.set(_ZUSTAND_TEXT[RPiZA.S_CONFIG])
        else:
